 ...
 */
const FilterColumnTable: FC<IFilterTable> = ({
  column,
  sortedUniqueValues,
}) => {
  const excelFilters = (column.getFilterValue() ?? []) as string[]

  const { table, tableRef, rowVirtualizer } = useFilterTable(
    sortedUniqueValues as string[],
  )
  ...
  return (
    <div ref={tableRef} className="relative h-[180px] w-full overflow-auto">
      <div
          ...
          return (
            ...
            >
              {visibleCells.map((cell) => {
                ...
                const isSelected = excelFilters.find(
                  (excelItem) => excelItem === item,
                )
